#!/usr/bin/env python3
"""
Del-resilient hypermedia pagination
"""
...
class Server:
    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        """
        Items from dataset are not misses when changing page.
        Args:
        ...
        [keys.append(i)
         for i in keys if i >= index and len(keys) <= page_size]
        data = [dataset[v] for v in keys[:-1]]
        next_index = keys[-1] if len(keys) - page_size == 1 else None
